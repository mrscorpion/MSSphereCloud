...
- (void)setCloudTags:(NSArray *)array
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++)
    {
        ...
        MSPoint point = MSPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(MSPoint)];
    ...
    normalDirection = MSPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point
- (void)updateFrameOfPoint:(NSInteger)index direction:(MSPoint)direction andAngle:(CGFloat)angle
{
    NSValue *value = [coordinate objectAtIndex:index];
    MSPoint point;
    [value getValue:&point];
    MSPoint rPoint = MSPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(MSPoint)];
    coordinate[index] = value;
    [self setTagOfPoint:rPoint andIndex:index];
}

- (void)setTagOfPoint: (MSPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    ...
    else if (gesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        MSPoint direction = MSPointMake(last.y - current.y, current.x - last.x, 0);
